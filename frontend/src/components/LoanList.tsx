...
const LoanList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      try {
        const response = await fetch(`https://credit-sea-flax.vercel.app/loans/id?idNumber=${12140970}`);
        ...
        const updated_data = data.map(loan => ({
          ...
          amount: new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(loan.loanAmount), // Formatting loanAmount as Indian currency
